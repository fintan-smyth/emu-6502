...
void	mmc1_update_mappings(t_nes *nes, t_cart *cart)
{
	...
	switch (prg_mode) {
		case (2):
			map_memory(nes->cpu.pagetable, 0x8000, 16, &cart->prg_rom[0x4000 * (cart->prg_banks - 1)], NULL, mmc1_write_handler);
		...
		case (3):
			...
			map_memory(nes->cpu.pagetable, 0xC000, 16, &cart->prg_rom[0x4000 * (cart->prg_banks - 1)], NULL, mmc1_write_handler);
			break;
		default:
	...
	switch (chr_mode) {
		case (0):
			map_memory(nes->ppu.pagetable, 0x0000, 8, &cart->chr_ram[0x1000 * (chr_bank0 & 0x0E)], NULL, NULL);
			break;
		case (1):
			map_memory(nes->ppu.pagetable, 0x0000, 4, &cart->chr_ram[0x1000 * chr_bank0], NULL, NULL);
			map_memory(nes->ppu.pagetable, 0x1000, 4, &cart->chr_ram[0x1000 * chr_bank1], NULL, NULL);
			break;
	}
}
...
void	apply_mapper_mmap(t_nes *nes, t_cart *cart)
{
	switch (cart->mapper_id) {
		...
		case (2):
			...
			map_memory(nes->cpu.pagetable, 0xC000, 16, &cart->prg_rom[0x4000 * (cart->prg_banks - 1)], NULL, uxrom_write_handler);
			break;
		default:
